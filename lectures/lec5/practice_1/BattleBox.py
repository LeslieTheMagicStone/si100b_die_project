...
class BattleBox:
    def __init__(
        ...
        self.bg = pygame.Surface(
            (BattleSettings.boxWidth, BattleSettings.boxHeight), pygame.SRCALPHA
        )
        ...
        self.playerImg = pygame.transform.scale(
            player.image, (BattleSettings.playerWidth, BattleSettings.playerHeight)
        )
        ...
        self.monsterImg = pygame.transform.scale(
            monster.image, (BattleSettings.monsterWidth, BattleSettings.monsterHeight)
        )
    ...
    def get_result(self):
        if self.attacker == 0:
            self.monsterHP = max(
                0, self.monsterHP - (self.player.attack - self.monster.defence)
            )
            ...
            self.playerHP = max(
                0, self.playerHP - (self.monster.attack - self.player.defence)
            )
    ...
    def render(self):
        ...
        self.window.blit(
            self.font.render(text, True, self.fontColor),
        ...
        if self.playerHP == 0 or self.monsterHP == 0:
            if self.monsterHP == 0:
                ...
                self.window.blit(
                    self.font.render(text, True, self.fontColor),
                    (BattleSettings.textPlayerStartX, BattleSettings.textStartY+100),
                )
                self.monster.kill()
